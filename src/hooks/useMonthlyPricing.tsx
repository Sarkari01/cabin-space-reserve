...
export const useMonthlyPricing = () => {
  const { user } = useAuth();
  ...
  const getPricingPlan = async (studyHallId: string): Promise<MonthlyPricingPlan | null> => {
    try {
      ...
      console.error('Error in getPricingPlan:', error);
      return null;
    }
  };
  ...
  const savePricingPlan = async (pricingPlan: MonthlyPricingPlan): Promise<boolean> => {
    ...
    try {
      ...
      if (!hasValidPricing) {
        console.error('At least one pricing period must be enabled with a valid price');
        return false;
      }
      ...
      console.error('Error in savePricingPlan:', error);
      return false;
    } finally {
      setLoading(false);
    }
  ...
  const getMerchantPricingPlans = async (merchantId: string): Promise<MonthlyPricingPlan[]> => {
    try {
      ...
      console.error('Error in getMerchantPricingPlans:', error);
      return [];
    }
  };
