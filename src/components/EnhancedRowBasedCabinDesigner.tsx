...
export const EnhancedRowBasedCabinDesigner: React.FC<EnhancedRowBasedCabinDesignerProps> = ({
  ...
  const addRow = () => {
    ...
    setRows([...rows, { name: nextLetter, cabins: 5 }]);
  };
  ...
  const getCabinStatusInfo = (cabin: any) => {
    ...
    if (!cabinAvailability) return { status: 'available', color: 'bg-green-100 border-green-400', textColor: 'text-green-700' };

    switch (cabinAvailability.status) {
      case 'occupied':
        return { status: 'Occupied', color: 'bg-red-100 border-red-400', textColor: 'text-red-700' };
      case 'maintenance':
        return { status: 'Maintenance', color: 'bg-yellow-100 border-yellow-400', textColor: 'text-yellow-700' };
      default:
        return { status: 'Available', color: 'bg-green-100 border-green-400', textColor: 'text-green-700' };
    }
  };

  const totalCabins = rows.reduce((sum, row) => sum + row.cabins, 0);
  const totalRevenue = rows.reduce((sum, row) => 
    sum + (row.cabins * (row.priceOverride || basePrice)), 0
  );
  ...
  return (
    <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
              ...
              <div>
                ...
                <Input
                  ...
                  max="20"
                  value={row.cabins}
                  onChange={(e) => updateRow(index, { cabins: parseInt(e.target.value) || 1 })}
                />
              </div>
      ...
      <Card className="p-4">
        ...
        <div 
          className="relative bg-gradient-to-b from-slate-50 to-slate-100 border-2 border-dashed border-border rounded-lg overflow-auto"
          style={{ 
            width: '100%', 
            height: '500px',
          }}
          ...
          >
            {layout.cabins.map((cabin) => {
              ...
              return (
                <div
                  key={cabin.id}
                  className={`absolute border-2 rounded-lg text-xs flex flex-col justify-center items-center transition-all cursor-pointer hover:shadow-lg hover:scale-105 ${statusInfo.color}`}
                  style={{
                    left: cabin.x,
                    top: cabin.y,
                    width: cabin.width,
            ...
            <div className="absolute bottom-4 left-1/2 transform -translate-x-1/2 bg-gradient-to-r from-blue-500 to-purple-600 text-white px-4 py-2 rounded-lg text-sm font-medium shadow-lg">
              🚪 Main Entrance
            </div>
