-- Add 'institution' role to the user_role enum (if it doesn't exist)
DO $$ 
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_type t JOIN pg_enum e ON t.oid = e.enumtypid WHERE t.typname = 'user_role' AND e.enumlabel = 'institution') THEN
...
-- Add institution_id column to news_posts for attribution (if it doesn't exist)
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'news_posts' AND column_name = 'institution_id') THEN
...
DROP POLICY IF EXISTS "Institutions can view and update their own record" ON public.institutions;
DROP POLICY IF EXISTS "Institutions can manage their own news posts" ON public.news_posts;
