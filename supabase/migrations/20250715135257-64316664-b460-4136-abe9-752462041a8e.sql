...
ADD COLUMN max_study_halls integer default 1;

-- Update status enum to include trial
ALTER TYPE merchant_subscription_status ADD VALUE IF NOT EXISTS 'trial';

-- Add plan_type field to distinguish between basic, premium, trial
ALTER TABLE public.merchant_subscriptions 
ADD COLUMN plan_type text default 'basic' check (plan_type in ('basic', 'premium', 'trial'));
...
-- Function to auto-expire trial subscriptions
CREATE OR REPLACE FUNCTION public.auto_expire_trial_subscriptions()
RETURNS void
LANGUAGE plpgsql
AS $$
BEGIN
  ...
  RAISE LOG 'Auto-expired % trial subscriptions', (SELECT COUNT(*) FROM merchant_subscriptions WHERE is_trial = true AND trial_end_date < now() AND status = 'cancelled');
END;
$$;