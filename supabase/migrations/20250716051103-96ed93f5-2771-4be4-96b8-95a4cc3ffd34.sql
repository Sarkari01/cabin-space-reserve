-- Update get_active_popup_notifications function to handle both singular and plural role matching
...
BEGIN
  ...
  WHERE n.popup_enabled = true
    ...
    AND (
      ...
      (n.target_audience = 'students' AND p_user_role IN ('student', 'students')) OR
      (n.target_audience = 'merchants' AND p_user_role IN ('merchant', 'merchants')) OR
      (n.target_audience = 'student' AND p_user_role IN ('student', 'students')) OR
      (n.target_audience = 'merchant' AND p_user_role IN ('merchant', 'merchants'))
    )
    AND NOT EXISTS (
