...
-- Recreate the function with proper parameter naming and explicit column references
...
BEGIN
  ...
  -- Count current study halls (explicitly reference study_halls.status)
  SELECT COUNT(*) INTO current_halls
  FROM study_halls
  WHERE merchant_id = p_merchant_id
  AND study_halls.status = 'active';
