-- Safely replace function with new return signature by dropping first
DO $$
BEGIN
  IF EXISTS (
    SELECT 1 FROM pg_proc p
    JOIN pg_namespace n ON n.oid = p.pronamespace
    WHERE p.proname = 'get_eligible_transactions_for_settlement'
      AND n.nspname = 'public'
      AND pg_get_function_identity_arguments(p.oid) = 'p_merchant_id uuid'
  ) THEN
    DROP FUNCTION public.get_eligible_transactions_for_settlement(p_merchant_id uuid);
  END IF;
END $$;
