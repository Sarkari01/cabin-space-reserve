...
SELECT 
  ...
  END,
  CASE 
    WHEN random() < 0.5 THEN jsonb_build_object('payment_id', 'pay_' || gen_random_uuid()::text, 'order_id', 'order_' || gen_random_uuid()::text)
    ELSE jsonb_build_object('qr_id', 'qr_' || gen_random_uuid()::text, 'transaction_ref', 'txn_' || gen_random_uuid()::text)
  END,
