...
-- Add new RLS policy for admins to view all study halls (with unique name)
CREATE POLICY "Admin full access to view study halls" 
...
-- Add policy for admins to update any study hall status (with unique name)
CREATE POLICY "Admin full access to update study halls" 
ON public.study_halls 
FOR UPDATE 
TO authenticated
USING (public.is_admin())
