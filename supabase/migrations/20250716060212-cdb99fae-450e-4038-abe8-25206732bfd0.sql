-- Update get_active_popup_notifications function to allow admins to see all notifications
...
BEGIN
  ...
  WHERE n.popup_enabled = true
    ...
    AND (
      -- Admins can see all notifications regardless of target audience
      p_user_role = 'admin' OR
      n.target_audience = 'all_users' OR
      n.target_audience = p_user_role OR
      (n.target_audience = 'students' AND p_user_role = 'student') OR
      (n.target_audience = 'merchants' AND p_user_role = 'merchant') OR
      (n.target_audience = 'student' AND p_user_role = 'student') OR
      (n.target_audience = 'merchant' AND p_user_role = 'merchant')
    )
    AND NOT EXISTS (
