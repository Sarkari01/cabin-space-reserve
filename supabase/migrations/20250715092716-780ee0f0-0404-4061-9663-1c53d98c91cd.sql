-- Phase 1: Enhanced seat management system (Fixed version)
...
BEGIN
  ...
  IF TG_OP = 'UPDATE' AND existing_booking_ids IS NOT NULL THEN
    -- Try to restore bookings to similar seats
    UPDATE bookings 
    SET seat_id = (
      SELECT s.id 
      FROM seats s 
...
-- Enable real-time for seats table (only set replica identity if not already set)
